...
use Voyager\Contracts\Vessel\Vessel as VesselContract;
...
class Vessel implements ArrayAccess, VesselContract
{
    use ReflectsClosures;

    /**
     * The current globally available vessel (if any).
     *
     * @var static
     */
    protected static ?VesselContract $instance = null;
    ...
    /**
     * The callback used to determine the vessel's environment.
     *
     ...
     * @return \Voyager\Contracts\Vessel\ContextualBindingBuilder
     */
    ...
    /**
     * Register a binding with the vessel.
     *
    ...
    public function bind(Closure|string $abstract, Closure|string|null $concrete = null, bool $shared = false): void
    {
        ...
        // bound into this vessel to the abstract type and we will just wrap it
        ...
        // If the abstract type was already resolved in this vessel we'll fire the
    ...
    {
        return function ($vessel, $parameters = []) use ($abstract, $concrete) {
            if ($abstract == $concrete) {
                return $vessel->build($concrete);
            }

            return $vessel->resolve(
    ...
    /**
     * Determine if the vessel has a method binding.
     *
     * @param  string  $method
     * @return bool
     */
    ...
    /**
     * Add a contextual binding to the vessel.
     *
     * @param  string  $concrete
     * @param  \Closure|string  $abstract
     ...
     * @return void
     */
    public function addContextualBinding(string $concrete, Closure|string $abstract, $implementation): void
    {
    ...
    /**
     * Register a shared binding in the vessel.
     *
     * @param  \Closure|string  $abstract
     * @param  \Closure|string|null  $concrete
     * @return void
    ...
    /**
     * Register a binding with the vessel based on the given Closure's return types.
     *
     * @param  \Closure|string  $abstract
     * @param  \Closure|string|null  $concrete
     * @param  bool  $shared
    ...
    /**
     * "Extend" an abstract type in the vessel.
     *
     * @param  string  $abstract
     * @return void
     *
    ...
    /**
     * Register an existing instance as shared in the vessel.
     *
    ...
    public function instance(Closure|string $abstract, mixed $instance): mixed
    {
        ...
        // we will fire the rebound callbacks registered with the vessel and it
    ...
    {
        $tags = count($tags) === 1 && is_array($tags[0]) ? $tags[0] : $tags;
     ...
     * @return string|false
     */
    protected function getClassForCallable(callable|string|array $callback): string|false
    {
    ...
    /**
     * Get a closure to resolve the given type from the vessel.
     *
    ...
    /**
     * Resolve the given type from the vessel.
     *
     * @template TClass of object
     *
     * @param  string|class-string<TClass>  $abstract
     ...
     *
     * @throws \Voyager\Contracts\Vessel\BindingResolutionException
     * @throws \Voyager\Contracts\Vessel\CircularDependencyException
     */
    protected function resolve($abstract, $parameters = [], $raiseEvents = true)
    {
        // NOTE: no parameter or return types are declared here on purpose. This method
        // is overridden by Voyager\System\Application with an untyped signature, so
        // narrowing it here would break that subclass at load time.
        $abstract = $this->getAlias($abstract);
    ...
    protected function getConcrete($abstract): mixed
    {
        ...
        // since the vessel should be able to resolve concretes automatically.
     ...
     */
    protected function findInContextualBindings($abstract): mixed
    {
    ...
    /**
     * Resolve a class based dependency from the vessel.
     *
     * @return mixed
     *
     * @throws \Voyager\Contracts\Vessel\BindingResolutionException
    ...
    /**
     * Resolve a class based variadic dependency from the vessel.
     *
     * @return mixed
     */
    ...
    /**
     * Fire every after-resolving callback.
     *
     * @param  string  $abstract
     * @param  mixed  $object
     * @return void
    ...
    /**
     * Get the name of the binding the vessel is currently resolving.
     *
    ...
    /**
     * Remove every extender callback for a given type.
     *
     * @param  string  $abstract
     * @return void
     */
    ...
    /**
     * Clear every instance from the vessel.
     *
     * @return void
     */
    ...
    /**
     * Clear every scoped instance from the vessel.
     *
     * @return void
     */
    ...
    /**
     * Determine the environment for the vessel.
     *
    ...
    /**
     * Get the globally available instance of the vessel.
     *
     * @return static
     */
    ...
    /**
     * Set the shared instance of the vessel.
     *
     * @return \Voyager\Contracts\Vessel\Vessel|static
     */
    public static function setInstance(?VesselContract $vessel = null): ?VesselContract
    {
        return static::$instance = $vessel;
    }
    ...
    /**
     * Dynamically access vessel services.
     *
     * @param  string  $key
     * @return mixed
     */
    ...
    /**
     * Dynamically set vessel services.
     *
     * @param  string  $key
     * @param  mixed  $value
     * @return void
