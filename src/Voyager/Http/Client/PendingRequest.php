...
use Voyager\Http\Client\Events\ConnectionFailed;
use Voyager\Http\Client\Events\RequestSending;
use Voyager\Http\Client\Events\ResponseReceived;
use Voyager\Http\Client\Promises\FluentPromise;
use Voyager\Http\Client\Promises\LazyPromise;
...
class PendingRequest
{
     ...
     *
     * @var \Voyager\Http\Client\Factory|null
     */
    protected $factory;
     ...
     *
     * @var \Voyager\NutsAndBolts\Collection<int, (callable(\Voyager\Http\Client\Response): \Voyager\Http\Client\Response|null)>
     */
     ...
     *
     * @var \Voyager\NutsAndBolts\Collection|null
     */
     ...
     *
     * @var \Voyager\Http\Client\Request|null
     */
    protected $request;
     ...
     *
     * @param  (callable(\Voyager\Http\Client\Response): \Voyager\Http\Client\Response|null)  $callback
     * @return $this
     */
     ...
     * @return \Voyager\Http\Client\Response|\GuzzleHttp\Promise\PromiseInterface
     *
     * @phpstan-return (TAsync is false ?  \Voyager\Http\Client\Response : \GuzzleHttp\Promise\PromiseInterface)
     *
     ...
     *
     * @param  string  $url
     * @param  array|\JsonSerializable|\Voyager\Contracts\NutsAndBolts\Arrayable  $data
     * @return \Voyager\Http\Client\Response|\GuzzleHttp\Promise\PromiseInterface
     *
     * @phpstan-return (TAsync is false ?  \Voyager\Http\Client\Response : \GuzzleHttp\Promise\PromiseInterface)
     *
     ...
     *
     * @param  (callable(\Voyager\Http\Client\Pool): mixed)  $callback
     * @param  non-negative-int|null  $concurrency
     * @return array<array-key, \Voyager\Http\Client\Response|\Throwable>
     */
     ...
     *
     * @param  callable  $callback
     * @return \Voyager\Http\Client\Batch
     */
     ...
     *
     * @param  \Voyager\Http\Client\Response|\Throwable  $response
     * @param  string  $method
     * @param  string  $url
     * @param  array  $options
     * @param  int  $attempt
    ...
    protected function sendRequest(string $method, string $url, array $options = [])
    {
        ...
        $venusianData = $this->parseRequestData($method, $url, $options);
    ...
    protected function parseRequestData($method, $url, array $options)
    {
        ...
        $venusianData = $options[$this->bodyFormat] ?? $options['query'] ?? [];

        $urlString = (new Stringable($url));

        if (empty($venusianData) && $method === 'GET' && $urlString->contains('?')) {
            $venusianData = (string) $urlString->after('?');
        }

        if (is_string($venusianData)) {
            parse_str($venusianData, $parsedData);

            $venusianData = is_array($parsedData) ? $parsedData : [];
        }

        if ($venusianData instanceof JsonSerializable) {
            $venusianData = $venusianData->jsonSerialize();
        }

        return is_array($venusianData) ? $venusianData : [];
    }
     ...
     * @return \Voyager\Http\Client\Response
     */
     ...
     * @return \Closure
     *
     * @throws \Voyager\Http\Client\Exceptions\StrayRequestException
     */
    ...
    {
        return tap(new Response($response), function (Response $venusianResponse) {
            if ($this->truncateExceptionsAt === null) {
                return;
            }

            $this->truncateExceptionsAt === false
                ? $venusianResponse->dontTruncateExceptions()
                : $venusianResponse->truncateExceptionsAt($this->truncateExceptionsAt);
        });
    }
     ...
     * @return void
     *
     * @throws \Voyager\Http\Client\RequestException
     * @throws \Voyager\Http\Client\ConnectionException
     */
