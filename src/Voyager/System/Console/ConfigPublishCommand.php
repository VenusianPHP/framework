...
class ConfigPublishCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        if (is_null($this->argument('name')) && $this->option('all')) {
            foreach ($config as $key => $file) {
                $this->publish($key, $file, $this->venusian->configPath().'/'.$key.'.php');
            }
        ...
        $name = (string) (is_null($this->argument('name')) ? select(
            ...
            // ->all(): laravel/prompts types $options against Illuminate's
            // Collection, which Voyager's is not. Hand it the array.
            options: (new Collection($config))->map(fn (string $path) => basename($path, '.php'))->all(),
        ) : $this->argument('name'));
        ...
        $this->publish($name, $config[$name], $this->venusian->configPath().'/'.$name.'.php');
    ...
    protected function getBaseConfigurationFiles(): array
    {
        ...
        $shouldMergeConfiguration = $this->venusian->shouldMergeFrameworkConfiguration();
