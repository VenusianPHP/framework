...
class EnvironmentDecryptCommand extends Command
{
    ...
    public function handle(): void
    {
        ...
        $encryptedFile = ($this->option('env')
            ? Str::finish($this->venusian->environmentPath(), DIRECTORY_SEPARATOR).'.env.'.$this->option('env')
            : $this->venusian->environmentFilePath()).'.encrypted';
     ...
     *
     * @param  string  $contents
     * @param  \Voyager\Encryption\Encrypter  $encrypter
     * @return string
     */
    ...
    {
        $path = Str::finish($this->option('path') ?: $this->venusian->environmentPath(), DIRECTORY_SEPARATOR);
