...
use Exception;
use Voyager\Contracts\Core\FrameworkCore as ApplicationContract;
...
class ProviderRepository
{
     ...
     */
    public function __construct(ApplicationContract $app, Filesystem $files, string $manifestPath)
    {
        $this->app = $app;
        $this->files = $files;
    ...
    {
        $results = is_null($manifest) || ($manifest['providers'] != $providers);
        return $results;
    }
    ...
    protected function registerLoadEvents(string $provider, array $events): void
    {
        ...
        $this->app->make('signals')->listen($events, fn () => $this->app->register($provider));
    }
