...
`Loop::await()` waits on the `Promise` contract, which includes `Task`. Checking the concrete `Voyager\IOPools\Promise` class instead wraps a `Task` as a foreign thenable; `Task::then()` takes one callback, so a rejection never settles the wrapper and `until()` runs out of work.[^loop]
