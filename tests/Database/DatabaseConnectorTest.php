...
use Voyager\Database\Connectors\PostgresConnector;
use Voyager\Database\Connectors\SQLiteConnector;
use Voyager\Database\Connectors\SqlServerConnector;
use Mockery as m;
use PDO;
use PDOStatement;
use stdClass;

function dbConnectorGetDsn(array $config)
{
...
test('option resolution', function () {
    $connector = new Connector;
    $connector->setDefaultOptions([0 => 'foo', 1 => 'bar']);
    expect($connector->getOptions(['options' => [0 => 'baz', 2 => 'boom']]))->toEqual([0 => 'baz', 1 => 'bar', 2 => 'boom']);
});

test('my sql connect calls create connection with proper arguments', function ($dsn, $config) {
...
test('my sql connect calls create connection with isolation level', function () {
...
test('postgres connect calls create connection with proper arguments', function () {
 ...
 */
test('postgres search path is set', function ($searchPath, $expectedSql) {
...
test('postgres search path fallback to config key schema', function () {
    $dsn = 'pgsql:host=foo;dbname=\'bar\';client_encoding=\'utf8\'';
...
test('postgres application name is set', function () {
...
test('postgres application use alternative database name', function () {
...
test('postgres application use alternative database name and port', function () {
...
test('postgres connector reads isolation level from config', function () {
...
test('s q lite memory databases may be connected to', function () {
    $dsn = 'sqlite::memory:';
...
test('s q lite named memory databases may be connected to', function () {
...
test('s q lite file databases may be connected to', function () {
    $dsn = 'sqlite:'.__DIR__;
...
test('sql server connect calls create connection with proper arguments', function () {
    $config = ['host' => 'foo', 'database' => 'bar', 'port' => 111];
    $dsn = dbConnectorGetDsn($config);
...
test('sql server connect calls create connection with optional arguments', function () {
...
test('sql server connect calls create connection with preferred o d b c', function () {
    ...
    expect($result)->toBe($connection);
})->skip(fn () => ! in_array('odbc', PDO::getAvailableDrivers()), 'PHP extension odbc is required.');
