...
function schemaBlueprintConnection(?string $grammar = null, string $prefix = '')
{
    ...
    $grammar ??= 'MySql';
    $grammarClass = 'Voyager\\Database\\Schema\\Grammars\\'.$grammar.'Grammar';
    $builderClass = 'Voyager\\Database\\Schema\\'.$grammar.'Builder';
...
function schemaBlueprintBlueprint(
    ?string $grammar = null,
    string $table = '',
    ?Closure $callback = null,
    string $prefix = ''
): Blueprint {
    $connection = schemaBlueprintConnection($grammar, $prefix);
...
test('to sql runs commands from blueprint', function () {
    $conn = schemaBlueprintConnection();
...
test('index default names', function () {
    $blueprint = schemaBlueprintBlueprint(table: 'users');
    ...
    $blueprint = schemaBlueprintBlueprint(table: 'geo');
    $blueprint->spatialIndex('coordinates');
    $commands = $blueprint->getCommands();
    $this->assertSame('geo_coordinates_spatialindex', $commands[0]->index);
});

test('index default names when prefix supplied', function () {
    $blueprint = schemaBlueprintBlueprint(table: 'users', prefix: 'prefix_');
    $blueprint->unique(['foo', 'bar']);
    $commands = $blueprint->getCommands();
    ...
    $blueprint = schemaBlueprintBlueprint(table: 'geo', prefix: 'prefix_');
    $blueprint->spatialIndex('coordinates');
    $commands = $blueprint->getCommands();
    $this->assertSame('prefix_geo_coordinates_spatialindex', $commands[0]->index);
});

test('drop index default names', function () {
    $blueprint = schemaBlueprintBlueprint(table: 'users');
...
test('drop index default names when prefix supplied', function () {
    $blueprint = schemaBlueprintBlueprint(table: 'users', prefix: 'prefix_');
    $blueprint->dropUnique(['foo', 'bar']);
    $commands = $blueprint->getCommands();
    $this->assertSame('prefix_users_foo_bar_unique', $commands[0]->index);
...
test('default current date', function () {
    $getSql = function ($grammar, $mysql57 = false) {
        if ($grammar == 'MySql') {
            $connection = schemaBlueprintConnection($grammar);
            ...
            return schemaBlueprintBlueprint($grammar, 'users', function ($table) {
                $table->date('created')->useCurrent();
            })->toSql();
        }
    };
...
test('default current date time', function () {
...
test('default current timestamp', function () {
    $getSql = function ($grammar) {
        return schemaBlueprintBlueprint($grammar, 'users', function ($table) {
...
test('default current year', function () {
    $getSql = function ($grammar, $mysql57 = false) {
        if ($grammar == 'MySql') {
            $connection = schemaBlueprintConnection($grammar);
            $mysql57 ? $connection->shouldReceive('getServerVersion')->andReturn('5.7') : $connection->shouldReceive('getServerVersion')->andReturn('8.0.13');
...
test('native rename column on mysql57', function () {
    $connection = schemaBlueprintConnection('MySql');
    $connection->shouldReceive('isMaria')->andReturn(false);
...
test('native rename column on legacy mariadb', function () {
    $connection = schemaBlueprintConnection('MariaDb');
...
test('native column modifying on mysql', function () {
    $blueprint = schemaBlueprintBlueprint('MySql', 'users', function ($table) {
...
test('default using id morph', function () {
    $getSql = function ($grammar) {
        return schemaBlueprintBlueprint($grammar, 'comments', function ($table) {
...
test('default using nullable id morph', function () {
    $getSql = function ($grammar) {
        return schemaBlueprintBlueprint($grammar, 'comments', function ($table) {
...
test('default using uuid morph', function () {
...
test('default using nullable uuid morph', function () {
    Builder::defaultMorphKeyType('uuid');
...
test('default using ulid morph', function () {
...
test('default using nullable ulid morph', function () {
    Builder::defaultMorphKeyType('ulid');
...
test('generate relationship column with non incremental model', function () {
    $getSql = function ($grammar) {
        return schemaBlueprintBlueprint($grammar, 'posts', function ($table) {
            $table->foreignIdFor(Fixtures\Models\InstrumentModelUsingNonIncrementedInt::class);
        })->toSql();
    };
...
test('generate relationship column with uuid model', function () {
    $getSql = function ($grammar) {
        return schemaBlueprintBlueprint($grammar, 'posts', function ($table) {
            $table->foreignIdFor(Fixtures\Models\InstrumentModelUsingUuid::class);
        })->toSql();
    };
...
test('generate relationship column with ulid model', function () {
    $getSql = function ($grammar) {
        return schemaBlueprintBlueprint($grammar, 'posts', function ($table) {
            $table->foreignIdFor(Fixtures\Models\InstrumentModelUsingUlid::class);
        })->toSql();
    };
...
test('generate relationship for model with non standard primary key name', function () {
    $getSql = function ($grammar) {
        return schemaBlueprintBlueprint($grammar, 'posts', function ($table) {
...
test('drop relationship column with uuid model', function () {
    $getSql = function ($grammar) {
        return schemaBlueprintBlueprint($grammar, 'posts', function ($table) {
            $table->dropForeignIdFor(Fixtures\Models\InstrumentModelUsingUuid::class);
        })->toSql();
    };
...
test('drop constrained relationship column with uuid model', function () {
    $getSql = function ($grammar) {
        return schemaBlueprintBlueprint($grammar, 'posts', function ($table) {
            $table->dropConstrainedForeignIdFor(Fixtures\Models\InstrumentModelUsingUuid::class);
        })->toSql();
    };
...
test('tiny text column', function () {
    $getSql = function ($grammar) {
        return schemaBlueprintBlueprint($grammar, 'posts', function ($table) {
            $table->tinyText('note');
        })->toSql();
...
test('tiny text nullable column', function () {
    $getSql = function ($grammar) {
        return schemaBlueprintBlueprint($grammar, 'posts', function ($table) {
