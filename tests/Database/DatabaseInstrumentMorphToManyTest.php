...
function dbMorphToManyGetRelation(): MorphToMany
{
    [$builder, $parent] = dbMorphToManyGetRelationArguments();

    return new MorphToMany($builder, $parent, 'taggable', 'taggables', 'taggable_id', 'tag_id', 'id', 'id');
}

function dbMorphToManyGetRelationArguments(): array
{
...
test('eager constraints are properly added', function () {
    $relation = dbMorphToManyGetRelation();
    ...
    $model1 = new InstrumentMorphToManyModelStub;
    $model1->id = 1;
    $model2 = new InstrumentMorphToManyModelStub;
    $model2->id = 2;
    $relation->addEagerConstraints([$model1, $model2]);
});

test('attach inserts pivot table record', function () {
    $relation = $this->getMockBuilder(MorphToMany::class)->onlyMethods(['touchIfTouching'])->setConstructorArgs(dbMorphToManyGetRelationArguments())->getMock();
...
test('detach removes pivot table record', function () {
    $relation = $this->getMockBuilder(MorphToMany::class)->onlyMethods(['touchIfTouching'])->setConstructorArgs(dbMorphToManyGetRelationArguments())->getMock();
    $query = m::mock(stdClass::class);
    $query->shouldReceive('from')->once()->with('taggables')->andReturn($query);
    ...
    expect($relation->detach([1, 2, 3]))->toBeTrue();
});

test('detach method clears all pivot records when no i ds are given', function () {
    $relation = $this->getMockBuilder(MorphToMany::class)->onlyMethods(['touchIfTouching'])->setConstructorArgs(dbMorphToManyGetRelationArguments())->getMock();
    $query = m::mock(stdClass::class);
    $query->shouldReceive('from')->once()->with('taggables')->andReturn($query);
    $query->shouldReceive('where')->once()->with('taggables.taggable_id', 1)->andReturn($query);
    ...
    expect($relation->detach())->toBeTrue();
});

test('query expression can be passed to different pivot query builder clauses', function () {
    $value = 'pivot_value';
...
class InstrumentMorphToManyModelStub extends Model
{
    protected $guarded = [];
}
