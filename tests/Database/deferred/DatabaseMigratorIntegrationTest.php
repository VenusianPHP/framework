...
test('basic migration of single folder', function () {
...
test('migrations default connection can be changed', function () {
...
test('migrations can each define connection', function () {
...
test('migrator cannot change defined migration connection', function () {
    $ran = $this->migrator->usingConnection('sqlite2', function () {
...
test('migrations can be reset using an string', function () {
    $this->migrator->run([__DIR__.'/migrations/one']);
    $this->assertTrue($this->db->schema()->hasTable('users'));
    $this->assertTrue($this->db->schema()->hasTable('password_resets'));
...
test('migrations can be reset using an array', function () {
    $this->migrator->run([__DIR__.'/migrations/one']);
    $this->assertTrue($this->db->schema()->hasTable('users'));
    $this->assertTrue($this->db->schema()->hasTable('password_resets'));
    $rolledBack = $this->migrator->reset([__DIR__.'/migrations/one']);
...
test('no error is thrown when no outstanding migrations exist', function () {
    $this->migrator->run([__DIR__.'/migrations/one']);
    $this->assertTrue($this->db->schema()->hasTable('users'));
    $this->assertTrue($this->db->schema()->hasTable('password_resets'));
    $this->migrator->run([__DIR__.'/migrations/one']);
...
test('no error is thrown when nothing to rollback', function () {
    $this->migrator->run([__DIR__.'/migrations/one']);
    $this->assertTrue($this->db->schema()->hasTable('users'));
    $this->assertTrue($this->db->schema()->hasTable('password_resets'));
...
test('migrations can run across multiple paths', function () {
...
test('migrations can be rolled back across multiple paths', function () {
    $this->migrator->run([__DIR__.'/migrations/one', __DIR__.'/migrations/two']);
    $this->assertTrue($this->db->schema()->hasTable('users'));
    $this->assertTrue($this->db->schema()->hasTable('password_resets'));
    $this->assertTrue($this->db->schema()->hasTable('flights'));
...
test('migrations can be reset across multiple paths', function () {
    $this->migrator->run([__DIR__.'/migrations/one', __DIR__.'/migrations/two']);
    $this->assertTrue($this->db->schema()->hasTable('users'));
    $this->assertTrue($this->db->schema()->hasTable('password_resets'));
    $this->assertTrue($this->db->schema()->hasTable('flights'));
...
test('migrations can be properly sorted across multiple paths', function () {
...
test('connection prior to migration is not changed after migration', function () {
...
test('connection prior to migration is not changed after rollback', function () {
    $this->migrator->setConnection('default');
    $this->migrator->run([__DIR__.'/migrations/one'], ['database' => 'sqlite2']);
...
test('connection prior to migration is not changed when no outstanding migrations exist', function () {
    $this->migrator->setConnection('default');
    $this->migrator->run([__DIR__.'/migrations/one'], ['database' => 'sqlite2']);
    $this->migrator->setConnection('default');
    $this->migrator->run([__DIR__.'/migrations/one'], ['database' => 'sqlite2']);
...
test('connection prior to migration is not changed when nothing to rollback', function () {
    $this->migrator->setConnection('default');
    $this->migrator->run([__DIR__.'/migrations/one'], ['database' => 'sqlite2']);
    $this->migrator->rollback([__DIR__.'/migrations/one'], ['database' => 'sqlite2']);
    $this->migrator->rollback([__DIR__.'/migrations/one'], ['database' => 'sqlite2']);
...
test('connection prior to migration is not changed after migrate reset', function () {
    $this->migrator->setConnection('default');
    $this->migrator->run([__DIR__.'/migrations/one'], ['database' => 'sqlite2']);
