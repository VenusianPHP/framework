...
function dbBtmCastedAttributesGetRelation()
{
    ...
    return new BelongsToMany(
        $builder,
        new InstrumentBelongsToManyModelStub,
        'relation',
        'foreign_key',
        'id',
        'parent_key',
...
test('models are properly matched to parents', function () {
    $relation = dbBtmCastedAttributesGetRelation();
    ...
    expect($models[1]->foo)->toBeNull();
    expect($models[0]->foo->count())->toBe(1);
    expect($models[0]->foo)->toContain($result1);
});

class InstrumentBelongsToManyModelStub extends Model
{
