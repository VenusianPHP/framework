...
use Venusian\Tests\Log\Fixtures\MyAddContextProcessor;
use Venusian\Tests\Log\Fixtures\StringBackedSuit;
use Venusian\Tests\Log\Fixtures\Suit;
...
function logApp(array $logging = []): RenderedInstance
{
    ...
    $app->registerInstance('config', new ConfigRepository(['logging' => $logging + [
        'default' => 'single',
...
function contextApp(bool $fresh = false): RenderedInstance
{
    static $app = null;

    if ($fresh || is_null($app)) {
        $app = logApp([
            'channels' => [
                'single' => [
                    'driver' => 'single',
...
function contextRepository(): Repository
{
    return contextApp()->make(Repository::class);
}
...
class Log
{
    public static function __callStatic(string $method, array $arguments): mixed
    {
        return contextApp()->make('log')->{$method}(...$arguments);
    }
}

class Event
{
    public static function dispatch(object|string $event, mixed $payload = [], bool $halt = false): mixed
    {
        return contextApp()->make('signals')->dispatch($event, $payload, $halt);
    }
}
    ...
    test('it can serialize values', function () {
        ...
        expect($dehydrated)->toBe([
            'data' => [
                ...
                'enum' => 'E:38:"Venusian\Tests\Log\Fixtures\Suit:Clubs";',
                'backed_enum' => 'E:50:"Venusian\Tests\Log\Fixtures\StringBackedSuit:Clubs";',
            ],
            'hidden' => [
                'number' => 'i:55;',
            ],
...
describe('logging', function () {
    ...
    test('it adds context to logged exceptions', function () {
        ...
        contextApp()->make(\Voyager\Contracts\Debug\ExceptionHandler::class)->report(new Exception('Whoops!'));
        $log = contextLogLine($path);
    ...
    test('the context log processor can be rebound to a separate class', function () {
        ...
        contextApp()->bind(ContextLogProcessor::class, MyAddContextProcessor::class);
