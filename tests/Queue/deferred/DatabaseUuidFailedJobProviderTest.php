...
function deferredUuidFailedJobProvider(string $database = 'default', string $table = 'failed_jobs'): DatabaseUuidFailedJobProvider
{
    $db = new DB;
    $db->addConnection([
        'driver' => 'sqlite',
...
test('getting ids of all failed jobs', function () {
    $provider = deferredUuidFailedJobProvider();
