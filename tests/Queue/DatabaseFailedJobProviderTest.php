...
function databaseFailedJobsTable()
{
    return test()->db->getConnection()->table('failed_jobs');
}

function createDatabaseFailedJobsRecord(array $overrides = [])
{
    return databaseFailedJobsTable()->insert(array_merge([
        'connection' => 'database',
        'queue' => 'default',
...
test('can get all failed job ids', function () {
    ...
    array_map(fn () => createDatabaseFailedJobsRecord(), range(1, 4));
...
test('can retrieve failed jobs by id', function () {
    array_map(fn () => createDatabaseFailedJobsRecord(), range(1, 2));
...
test('can remove failed jobs by id', function () {
    createDatabaseFailedJobsRecord();

    expect($this->provider->forget(2))->toBeFalse();
    expect(databaseFailedJobsTable()->count())->toBe(1);
    expect($this->provider->forget(1))->toBeTrue();
    expect(databaseFailedJobsTable()->count())->toBe(0);
});

test('can prune failed jobs', function () {
    Carbon::setTestNow(Carbon::createFromDate(2024, 4, 28));

    createDatabaseFailedJobsRecord(['failed_at' => Carbon::createFromDate(2024, 4, 24)]);
    createDatabaseFailedJobsRecord(['failed_at' => Carbon::createFromDate(2024, 4, 26)]);

    $this->provider->prune(Carbon::createFromDate(2024, 4, 23));
    expect(databaseFailedJobsTable()->count())->toBe(2);
...
test('can prune failed jobs with relative hours and minutes', function () {
    ...
    createDatabaseFailedJobsRecord(['failed_at' => Carbon::create(2025, 8, 24, 11, 45, 0)]);
    createDatabaseFailedJobsRecord(['failed_at' => Carbon::create(2025, 8, 24, 13, 0, 0)]);
...
test('can flush failed jobs', function () {
    ...
    createDatabaseFailedJobsRecord(['failed_at' => Date::now()->subDays(10)]);
    $this->provider->flush();
    expect(databaseFailedJobsTable()->count())->toBe(0);
...
test('can properly log failed job', function () {
    ...
    expect(databaseFailedJobsTable()->count())->toBe(1);
    expect(databaseFailedJobsTable()->first()->exception)->toBe($exception);
});
