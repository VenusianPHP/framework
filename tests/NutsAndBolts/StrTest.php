...
use Tests\NutsAndBolts\Fixtures\StringableObjectStub;
...
/*
|--------------------------------------------------------------------------
| Ported from Laravel's SupportStrTest (Laravel 12.67.0)
|--------------------------------------------------------------------------
|
| The block above is Venusian's own hand-written Str coverage. Everything
| below is a direct port of Laravel's upstream Str test suite, kept mostly
| verbatim (PHPUnit assertions work unchanged inside Pest's test() closures)
| so future diffs against upstream stay easy to read.
|
*/
...
dataset('strSpecialCharacters', function () {
    return [
...
test('stringCanBeLimitedByWords', function () {
...
test('stringCanBeLimitedByWordsNonAscii', function () {
...
test('stringTrimmedOnlyWhereNecessary', function () {
    $this->assertSame(' Taylor Otwell ', Str::words(' Taylor Otwell ', 3));
    $this->assertSame(' Taylor...', Str::words(' Taylor Otwell ', 1));
});

test('stringTitle', function () {
...
test('stringWithoutWordsDoesntProduceError', function () {
...
test('stringAsciiWithSpecificLocale', function () {
...
test('strContains', function ($haystack, $needles, $expected, $ignoreCase = false) {
...
test('strContainsAll', function ($haystack, $needles, $expected, $ignoreCase = false) {
...
test('strDoesntContain', function ($haystack, $needles, $expected, $ignoreCase = false) {
...
test('isWithMultilineStrings', function () {
    $this->assertFalse(Str::is('/', "/\n"));
    $this->assertTrue(Str::is('/*', "/\n"));
    $this->assertTrue(Str::is('*/*', "/\n"));
    $this->assertTrue(Str::is('*/*', "\n/\n"));
    ...
    namespace Voyager\Tests\NutsAndBolts;
    ...
    $this->assertTrue(Str::is("*namespace Voyager\Tests\*", $multilineValue));
    $this->assertFalse(Str::is("namespace Voyager\Tests\*", $multilineValue));
    $this->assertFalse(Str::is("*namespace Voyager\Tests", $multilineValue));
    $this->assertTrue(Str::is('<?php*', $multilineValue));
    $this->assertTrue(Str::is("<?php*namespace Voyager\Tests\*", $multilineValue));
    $this->assertFalse(Str::is('use Exception;', $multilineValue));
    $this->assertFalse(Str::is('use Exception;*', $multilineValue));
    $this->assertTrue(Str::is('*use Exception;', $multilineValue));

    $this->assertTrue(Str::is("<?php\n\nnamespace Voyager\Tests\*", $multilineValue));
...
test('isUuidWithValidUuid', function ($uuid) {
...
test('isUuidWithInvalidUuid', function ($uuid) {
...
test('isUuidWithVersion', function ($uuid, $version, $passes) {
...
test('whetherTheNumberOfGeneratedCharactersIsEquallyDistributed', function () {
    $results = [];
...
test('randomStringFactoryCanBeSet', function () {
...
test('itCanSpecifyASequenceOfRandomStringsToUtilise', function () {
...
test('itCanSpecifyAFallbackForARandomStringSequence', function () {
...
test('substrReplaceWithMultibyte', function () {
...
test('repeatWhenTimesIsNegative', function () {
    $this->expectException(ValueError::class);
    Str::repeat('Hello', -2);
});

test('transliterate', function (string $value, string $expected) {
    $this->assertSame($expected, Str::transliterate($value));
})->with('strSpecialCharacters');

test('transliterateOverrideUnknown', function () {
    $this->assertSame('HHH', Str::transliterate('🎂🚧🏆', 'H'));
    $this->assertSame('Hello', Str::transliterate('🎂', 'Hello'));
});

test('transliterateStrict', function (string $value, string $expected) {
    $this->assertSame($expected, Str::transliterate($value, '?', true));
})->with('strSpecialCharacters');

test('itCanFreezeUuids', function () {
...
test('itCanFreezeUuidsInAClosure', function () {
    $uuids = [];
...
test('itCreatesUuidsNormallyAfterFailureWithinFreezeMethod', function () {
    try {
...
test('itCanSpecifyASequenceOfUuidsToUtilise', function () {
...
test('itCanSpecifyAFallbackForASequence', function () {
...
test('itCanFreezeUlids', function () {
...
test('itCanFreezeUlidsInAClosure', function () {
    $ulids = [];
...
test('itCreatesUlidsNormallyAfterFailureWithinFreezeMethod', function () {
    try {
...
test('itCanSpecifyASequenceOfUlidsToUtilise', function () {
...
test('itCanSpecifyAFallbackForAUlidSequence', function () {
    Str::createUlidsUsingSequence(
        [Str::ulid(), Str::ulid()],
...
test('passwordCreation', function () {
