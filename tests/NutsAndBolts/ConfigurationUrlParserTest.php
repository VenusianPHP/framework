...
test('database urls are parsed', function ($config, $expectedOutput) {
    expect((new ConfigurationUrlParser)->parseConfiguration($config))->toEqual($expectedOutput);
})->with([
    'simple URL' => [
...
test('a driver alias may be registered and is then honoured', function () {
    expect(ConfigurationUrlParser::getDriverAliases())->toEqual([
        'mssql' => 'sqlsrv',
        'mysql2' => 'mysql',
        'postgres' => 'pgsql',
        'postgresql' => 'pgsql',
    ...
    ])->and((new ConfigurationUrlParser)->parseConfiguration('some-particular-alias://null'))->toEqual([
        'driver' => 'mysql',
    ]);
});
