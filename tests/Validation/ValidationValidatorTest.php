...
use Voyager\Validation\DatabasePresenceVerifierInterface;
use Voyager\Validation\Rule as ValidationRule;
...
function validatorArrayTranslator()
{
...
function validatorMockTranslator()
{
...
test('nested error messages are retrieved from local array', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->errors()->all()[0])->toBe('post name is required');
    });

test('nested array error messages are retrieved from local array', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, [
            'users' => [
                [
...
test('wildcard array custom messages handle missing rules gracefully when another rule fails', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, [
            'users' => [
                [
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->all()[0])->toBe('validation.in');
    });

test('sometimes works on nested arrays', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => ['bar' => ['baz' => '']]], ['foo.bar.baz' => 'sometimes|required']);
        expect($v->passes())->toBeFalse();
        expect($v->failed())->toEqual(['foo.bar.baz' => ['Required' => []]]);
...
test('after callbacks are called with validator instance', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => 'bar', 'baz' => 'boom'], ['foo' => 'Same:baz']);
        $v->setContainer(new Vessel);
        ...
        expect($v->passes())->toBeFalse();
        expect($_SERVER['__validator.after.test'])->toBeTrue();
        expect($v->errors()->has('bar'))->toBeTrue();

        unset($_SERVER['__validator.after.test']);
    });

test('sometimes works on arrays', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => ['bar', 'baz', 'moo']], ['foo' => 'sometimes|required|between:5,10']);
        expect($v->passes())->toBeFalse();
        expect($v->failed())->not->toBeEmpty();
...
test('validate throws on fail', function () {
...
test('validate doesnt throw on pass', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => 'bar'], ['foo' => 'required']);

        expect($v->validate())->toBe(['foo' => 'bar']);
    });

test('validated throws on fail', function () {
        $this->expectException(ValidationException::class);
...
test('validated throws on fail even after passes call', function () {
        $this->expectException(ValidationException::class);
...
test('validated doesnt throw on pass', function () {
        $trans = validatorArrayTranslator();

        $v = new Validator($trans, ['foo' => 'bar'], ['foo' => 'required']);
        expect($v->validated())->toBe(['foo' => 'bar']);

        $v = new Validator($trans, [], []);
        expect($v->validated())->toBe([]);

        $v = new Validator($trans, ['foo' => 'bar', 'baz' => 'qux'], ['foo' => 'required', 'baz' => 'required']);
        expect($v->validated())->toBe(['foo' => 'bar', 'baz' => 'qux']);
    });

test('has failed validation rules', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => 'bar', 'baz' => 'boom'], ['foo' => 'Same:baz']);
        expect($v->passes())->toBeFalse();
        expect($v->failed())->toEqual(['foo' => ['Same' => ['baz']]]);

        $v = new Validator($trans, ['foo' => 'bar', 'baz' => 'bar'], ['foo' => 'Same:baz', 'baz' => 'Required']);
        expect($v->passes())->toBeTrue();
        expect($v->failed())->toBeEmpty();
    });

test('failing once', function () {
        $trans = validatorArrayTranslator();
...
test('has not failed validation rules', function () {
        $trans = validatorMockTranslator();
...
test('sometimes can skip required rules', function () {
        $trans = validatorMockTranslator();
        $trans->shouldReceive('get')->never();
...
test('in validatable rules returns valid', function () {
        $trans = validatorMockTranslator();
        $trans->shouldReceive('get')->never();
        $v = new Validator($trans, ['foo' => 'taylor'], ['name' => 'Confirmed']);
        expect($v->passes())->toBeTrue();
...
test('validate using nested validation rules passes', function () {
        $rules = [
            'items' => ['array'],
        ...
        expect($v->messages()->get('items.0.|name')[0])->toBe('validation.in');
    });

test('validate empty strings always passes', function () {
        $trans = validatorArrayTranslator();
...
test('empty existing attributes are validated', function () {
        $trans = validatorArrayTranslator();
...
test('nullable', function () {
        ...
        expect($v->fails())->toBeTrue();
        expect($v->messages()->get('x')[0])->toBe('validation.string');
        expect($v->messages()->get('y')[0])->toBe('validation.integer');
        expect($v->messages()->get('z')[0])->toBe('validation.numeric');
        expect($v->messages()->get('a')[0])->toBe('validation.array');
        expect($v->messages()->get('b')[0])->toBe('validation.boolean');
    });

test('array nullable with unvalidated array keys', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeTrue();
        expect($v->validated())->toHaveKey('x');
...
test('nullable makes no difference if implicit rule exists', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->fails())->toBeTrue();
        expect($v->messages()->get('x')[0])->toBe('validation.integer');
        ...
        expect($v->fails())->toBeTrue();
        expect($v->messages()->get('y')[0])->toBe('validation.required_with');
    });

test('proper language line is set', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->messages()->first('name'))->toBe('required!');
    });

test('custom replacers are called', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name'))->toBe('foo taylor');
    });

test('class based custom replacers', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines(['validation.foo' => 'foo!'], 'en');
        $v = new Validator($trans, [], ['name' => 'required']);
        $v->setContainer($container = Mockery::mock(Vessel::class));
        $v->addReplacer('required', 'Foo@bar');
        $container->shouldReceive('make')->once()->with('Foo')->andReturn($foo = Mockery::mock(stdClass::class));
        $foo->shouldReceive('bar')->once()->andReturn('replaced!');
        $v->passes();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name'))->toBe('replaced!');
    });

test('nested attributes are replaced in dimensions', function () {
        ...
        $v->messages()->setFormat(':message');
        expect($v->fails())->toBeTrue();
        expect($v->messages()->first('x'))->toBe('10 20 1');
        ...
        $v->messages()->setFormat(':message');
        expect($v->fails())->toBeTrue();
        expect($v->messages()->first('x'))->toBe(':width :height 1');
    });

test('attribute names are replaced', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines(['validation.required' => ':attribute is required!'], 'en');
        $v = new Validator($trans, ['name' => ''], ['name' => 'Required']);
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name'))->toBe('name is required!');
...
test('attribute names are replaced in arrays', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines(['validation.required' => ':attribute is required!'], 'en');
        $v = new Validator($trans, ['users' => [['country_code' => 'US'], ['country_code' => null]]], ['users.*.country_code' => 'Required']);
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('users.1.country_code'))->toBe('users.1.country_code is required!');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name.1'))->toBe('Any name must be a string!');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('users.1.name'))->toBe('Any name must be a string!');

        $trans->addLines(['validation.required' => ':attribute is required!'], 'en');
        $v = new Validator($trans, ['title' => ['nl' => '', 'en' => 'Hello']], ['title.*' => 'required'], [], ['title.nl' => 'Titel', 'title.en' => 'Title']);
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('title.nl'))->toBe('Titel is required!');
        ...
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('names.0'))->toBe('names is required!');
        ...
        $v = new Validator($trans, ['names' => [null, 'name']], ['names.*' => 'Required']);
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('names.0'))->toBe('First name is required!');
    });

test('inline attribute names are replaced in arrays from nested rules', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines(['validation.required' => ':attribute is required!'], 'en');
        $v = new Validator($trans, [
            'users' => [['name' => 'Taylor']],
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('users.0.id'))->toBe('User ID is required!');
    });

test('translated attribute names are replaced in arrays from nested rules', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines([
...
test('translated attributes can be missing', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->messages()->first('total'))->toBe('total must be greater than 0.');
...
test('input is replaced', function () {
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('email'))->toBe('empty is not a valid email');
    });

test('capitalized displayable values are replaced', function () {
        // accepted_if
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('foo'))->toBe('The foo field must be accepted when Bar is Aaa.');
        ...
        expect($v->messages()->first('foo.2'))->toBe('The value of foo.2 does not exist in Bar.*.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->first('foo'))->toBe('The foo field must be missing when Baz / Bar is present.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->first('foo'))->toBe('The foo field must be present when Bar / Baz is present.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('email'))->toBe('The email field prohibits Emails / Email address being present.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('bar'))->toBe('The bar field is required when Foo is accepted.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('last'))->toBe('The last field is required unless First is in Taylor, Sven.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('lastName'))->toBe('The last name field is required unless first name is in tAylor, sVen.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('last'))->toBe('The last field is required when First is present.');
        ...
        $v = new Validator($trans, ['foo' => 'bar', 'baz' => 'boom'], ['foo' => 'Same:baz']);
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('foo'))->toBe('The foo field must match Baz.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('url'))->toBe('The url must start with one of the following values Http, Https');
...
test('input is replaced by its displayable value', function () {
        $frameworks = [
            1 => 'Laravel',
            2 => 'Symfony',
            3 => 'Rails',
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('framework'))->toBe('Rails is not a valid PHP Framework');
    });

test('displayable values are replaced', function () {
        // required_if:foo,bar
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('bar'))->toBe('The bar field is required when color is red.');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('bar'))->toBe('The bar field is required when subscribe is false.');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('bar'))->toBe('The bar field is required when subscribe is true.');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('bar'))->toBe('The bar field is required unless color is in red.');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('type'))->toBe('type must be included in Short, Long.');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('date'))->toBe('The date must be a date equal to the day after today.');
...
test('displayable attributes are replaced in custom replacers', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('lastname'))->toBe('Lastname needs to begin with the same letter as Firstname');
...
test('index values are replaced', function () {
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('input.*.attributes.*'))->toBe('Attribute (0, 1) (1, 2) must be a string.');

        $v = new Validator($trans, ['input' => [['name' => 'Bob'], ['name' => ''], ['name' => 'Jane']]], ['input.*.name' => 'required'], ['input.*.name.required' => 'Name :index is required.']);
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('input.*.name'))->toBe('Name 1 is required.');
...
test('position values are replaced', function () {
        $trans = validatorArrayTranslator();

        // $v = new Validator($trans, ['name' => ''], ['name' => 'required'], ['name.required' => 'Name :position is required.']);
        // expect($v->passes())->toBeFalse();
        // expect($v->messages()->first('name'))->toBe('Name 1 is required.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('input.*.name'))->toBe('Name 2 is required.');
...
test('ordinal position values are replaced', function () {
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('input.*.name'))->toBe('The 2nd Name is required.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('input.*.name'))->toBe('2nd Name is required.');
...
test('custom validation lines are respected', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name'))->toBe('really required!');
    });

test('custom validation lines for size rules', function () {
        $trans = validatorArrayTranslator();
        $trans->getLoader()->addMessages('en', 'validation', [
            'required' => 'required!',
            'custom' => [
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('image'))->toBe('Custom message for image filenames.');

        $file = new UploadedFile(__FILE__, '', null, null, true);
        $v = new Validator($trans, ['image' => $file], ['image' => 'gte:50']);
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('image'))->toBe('Custom message for image files.');
    });

test('custom validation lines are respected with asterisks', function () {
        $trans = validatorArrayTranslator();
        $trans->getLoader()->addMessages('en', 'validation', [
            'required' => 'required!',
            'custom' => [
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name.0'))->toBe('all are really required!');
        expect($v->messages()->first('name.1'))->toBe('all are really required!');
        expect($v->messages()->first('lang.en'))->toBe('english is required!');
    });
...
test('custom exception must extend validation exception', function () {
        $trans = validatorArrayTranslator();

        $v = new Validator($trans, [], []);

        $this->expectException(InvalidArgumentException::class);
        $this->expectExceptionMessage('Exception [RuntimeException] is invalid. It must extend [Voyager\Validation\ValidationException].');

        $v->setException(RuntimeException::class);
    });

test('validation dot custom dot anything can be translated', function () {
        $trans = validatorArrayTranslator();
        $trans->getLoader()->addMessages('en', 'validation', [
            'required' => 'required!',
            'custom' => [
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('validation.custom.0'))->toBe('should be integer!');
        expect($v->messages()->first('validation.custom.1'))->toBe('should be integer!');
    });

test('inline validation messages are respected', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['name' => ''], ['name' => 'Required'], ['name.required' => 'require it please!']);
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name'))->toBe('require it please!');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name'))->toBe('name should be of length 9');
    });

test('custom validation is appended to messages', function () {
        $trans = validatorArrayTranslator();
        ...
        ])->toEqual($validator->errors()->messages());
    });

test('inline validation messages are respected with asterisks', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['name' => ['', '']], ['name.*' => 'required|max:255'], ['name.*.required' => 'all must be required!']);
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name.0'))->toBe('all must be required!');
        expect($v->messages()->first('name.1'))->toBe('all must be required!');
    });

test('inline validation messages for rule objects are respected', function () {
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name'))->toBe('my custom message');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name.0'))->toBe('my custom message');
        expect($v->messages()->first('name.1'))->toBe('my custom message');

        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['name' => 'Ryan'], ['name' => $rule], [$rule::class => 'my attribute is :attribute']);
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name'))->toBe('my attribute is name');
    });

test('if rules are successfully added', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, [], ['foo' => 'Required']);
        // foo has required rule
        expect($v->hasRule('foo', 'Required'))->toBeTrue();
        // foo doesn't have array rule
        expect($v->hasRule('foo', 'Array'))->toBeFalse();
        // bar doesn't exists
        expect($v->hasRule('bar', 'Required'))->toBeFalse();
    });
...
test('validate array keys', function () {
        $trans = validatorArrayTranslator();
...
test('validation stops at failed presence check', function () {
        $trans = validatorArrayTranslator();

        $v = new Validator($trans, ['name' => null], ['name' => 'Required|string']);
        $v->passes();
        expect($v->errors()->get('name'))->toEqual(['validation.required']);

        $v = new Validator($trans, ['name' => null, 'email' => 'email'], ['name' => 'required_with:email|string']);
        $v->passes();
        expect($v->errors()->get('name'))->toEqual(['validation.required_with']);

        $v = new Validator($trans, ['name' => null, 'email' => ''], ['name' => 'required_with:email|string']);
        $v->passes();
        expect($v->errors()->get('name'))->toEqual(['validation.string']);

        $v = new Validator($trans, [], ['name' => 'present|string']);
        $v->passes();
        expect($v->errors()->get('name'))->toEqual(['validation.present']);
    });

test('validate present', function () {
        $trans = validatorArrayTranslator();
...
test('validate present if', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->first('foo'))->toBe('The foo field must be present when bar is 1.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->first('foo.0.id'))->toBe('The foo.0.id field must be present when bar is 1.');
...
test('validate present unless', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->first('foo'))->toBe('The foo field must be present unless bar is 1.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->first('foo.0.id'))->toBe('The foo.0.id field must be present unless bar is 1.');
...
test('validate present with', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines(['validation.present_with' => 'The :attribute field must be present when :values is present.'], 'en');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->first('foo.0.id'))->toBe('The foo.0.id field must be present when bar is present.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->first('foo'))->toBe('The foo field must be present when bar is present.');
    });

test('validate present with all', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->first('foo.0.id'))->toBe('The foo.0.id field must be present when bar / baz are present.');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->first('foo'))->toBe('The foo field must be present when bar / baz are present.');
    });
...
test('validate required with', function () {
        $trans = validatorArrayTranslator();
...
test('required with all', function () {
        $trans = validatorArrayTranslator();
...
test('validate required without', function () {
        $trans = validatorArrayTranslator();
...
test('required without multiple', function () {
        $trans = validatorArrayTranslator();
...
test('required without all', function () {
        $trans = validatorArrayTranslator();
...
test('required if', function () {
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('last'))->toBe('The last field is required when first is dayle.');
        ...
        expect($v->fails())->toBeTrue();
        expect($v->messages())->toHaveCount(1);
        expect($v->messages()->first('baz'))->toBe('The baz field is required when foo is 0.');
        ...
        expect($v->fails())->toBeTrue();
        expect($v->messages())->toHaveCount(1);
        expect($v->messages()->first('baz'))->toBe('The baz field is required when foo is empty.');
...
test('required if array to string conversation error exception', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, [
            'is_customer' => 1,
            'fullname' => null,
...
test('prohibited if', function () {
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('last'))->toBe('The last field is prohibited when first is jess.');
    });

test('validate prohibited accepted if', function () {
        $trans = validatorArrayTranslator();
        ...
        $v = new Validator($trans, ['foo' => 'true', 'bar' => 'baz'], ['bar' => 'prohibited_if_accepted:foo']);
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('bar'))->toBe('The bar field is prohibited when foo is accepted.');
    });

test('validate prohibited declined if', function () {
        $trans = validatorArrayTranslator();
        ...
        $v = new Validator($trans, ['foo' => 'false', 'bar' => 'baz'], ['bar' => 'prohibited_if_declined:foo']);
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('bar'))->toBe('The bar field is prohibited when foo is declined.');
    });

test('prohibited unless', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('last'))->toBe('The last field is prohibited unless first is in taylor, jess.');
    });
...
test('prohibits', function () {
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->has('foo.0.email'))->toBeTrue();
        expect($v->messages()->has('foo.1.email'))->toBeFalse();
    });

test('prohibited rules are consistent', function ($rules, $data, $result) {
        $trans = validatorArrayTranslator();

        expect((new Validator($trans, $data, $rules))->passes())->toBe($result);
    })->with('prohibitedRulesData');

test('failed file uploads', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->fails())->toBeTrue();
        expect($v->errors()->get('photo'))->toEqual(['validation.uploaded']);
        ...
        expect($v->fails())->toBeTrue();
        expect($v->errors()->get('photo'))->toEqual(['validation.string']);
...
test('validate in array', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => [1, 2, 3], 'bar' => [1, 2]], ['foo.*' => 'in_array:bar.*']);
        expect($v->passes())->toBeFalse();
...
test('validate hex color', function () {
        $trans = validatorArrayTranslator();
...
test('validate confirmed', function () {
        $trans = validatorArrayTranslator();
...
test('validate different', function () {
        $trans = validatorArrayTranslator();
...
test('validate accepted', function () {
        $trans = validatorArrayTranslator();
...
test('validate required accepted if', function () {
        $trans = validatorArrayTranslator();
...
test('validate accepted if', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => 'no', 'bar' => 'aaa'], ['foo' => 'accepted_if:bar,aaa']);
        expect($v->passes())->toBeFalse();
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('foo'))->toBe('The foo field must be accepted when bar is abc.');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('foo'))->toBe('The foo field must be accepted when bar is false.');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('foo'))->toBe('The foo field must be accepted when bar is true.');
    });

test('validate required if declined', function () {
        $trans = validatorArrayTranslator();
...
test('validate declined', function () {
        $trans = validatorArrayTranslator();
...
test('validate missing if', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->first('foo'))->toBe('The foo field must be missing when bar is 1.');
        ...
        expect($v->fails())->toBeTrue();
        expect($v->errors()->first('foo.0.baz'))->toBe('The foo.0.baz field must be missing when foo.0.bar is 1.');
    });

test('validate missing unless', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->first('foo'))->toBe('The foo field must be missing unless bar is 1.');
        ...
        expect($v->fails())->toBeTrue();
        expect($v->errors()->first('foo.0.baz'))->toBe('The foo.0.baz field must be missing unless foo.0.bar is 1.');
    });

test('validate missing with', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines(['validation.missing_with' => 'The :attribute field must be missing when :values is present.'], 'en');
        ...
        expect($v->fails())->toBeTrue();
        expect($v->errors()->first('foo.0.baz'))->toBe('The foo.0.baz field must be missing when foo.0.bar / foo.0.fred is present.');
    });

test('validate missing with all', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->first('foo'))->toBe('The foo field must be missing when baz / bar are present.');
        ...
        expect($v->fails())->toBeTrue();
        expect($v->errors()->first('foo.0.baz'))->toBe('The foo.0.baz field must be missing when foo.0.bar / foo.0.fred are present.');
    });

test('validate declined if', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('foo'))->toBe('The foo field must be declined when bar is aaa.');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('foo'))->toBe('The foo field must be declined when bar is abc.');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('foo'))->toBe('The foo field must be declined when bar is false.');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('foo'))->toBe('The foo field must be declined when bar is true.');
    });

test('validate ends with', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('url'))->toBe('The url must end with one of the following values http');

        $trans = validatorArrayTranslator();
        $trans->addLines(['validation.ends_with' => 'The :attribute must end with one of the following values :values'], 'en');
        $v = new Validator($trans, ['url' => 'laravel.com'], ['url' => 'ends_with:http,https']);
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('url'))->toBe('The url must end with one of the following values http, https');
    });

test('validate doesnt end with', function () {
        $trans = validatorArrayTranslator();
...
test('validate starts with', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('url'))->toBe('The url must start with one of the following values http');
...
test('validate doesnt start with', function () {
        $trans = validatorArrayTranslator();
...
test('validate boolean strict', function () {
        $trans = validatorArrayTranslator();
...
test('validate bool strict', function () {
        $trans = validatorArrayTranslator();
...
test('validate numeric', function () {
        $trans = validatorArrayTranslator();
...
test('validate numeric strict', function () {
        $trans = validatorArrayTranslator();
...
test('validate integer strict', function () {
        $trans = validatorArrayTranslator();
...
test('validate decimal', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->errors()->keys())->toBe(['decimal', 'scientific']);
        ...
        expect($v->errors()->keys())->toBe(['scientific']);
...
test('validate between', function () {
        $trans = validatorArrayTranslator();
...
test('validate multiple of', function ($input, $allowed, $passes) {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBe($passes);
        if ($v->fails()) {
            expect($v->messages()->first('foo'))->toBe("The foo must be a multiple of {$allowed}");
        } else {
            expect($v->messages()->first('foo'))->toBe('');
        }
    })->with('multipleOfDataProvider');

test('proper messages are returned for sizes', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name'))->toBe('numeric');

        $v = new Validator($trans, ['name' => 'asasdfadsfd'], ['name' => 'Size:2']);
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name'))->toBe('string');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('photo'))->toBe('file');
    });

test('validate gt place holder is replaced properly', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines([
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('items'))->toEqual(4);

        $v = new Validator($trans, ['items' => 3, 'more' => 5], ['items' => 'numeric|gt:more']);
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('items'))->toEqual(5);
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('max'))->toBe('minimum value');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('photo'))->toEqual(5);
...
test('validate lt place holder is replaced properly', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines([
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('items'))->toEqual(2);
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('min'))->toBe('maximum value');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('photo'))->toEqual(2);
...
test('validate gte place holder is replaced properly', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines([
...
test('validate lte place holder is replaced properly', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines([
...
test('validate contains', function () {
        $trans = validatorArrayTranslator();
...
test('validate distinct', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('foo.0'))->toBe('There is a duplication!');
        expect($v->messages()->first('foo.1'))->toBe('There is a duplication!');
...
test('validate distinct for top level arrays', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('0'))->toBe('There is a duplication!');
        expect($v->messages()->first('1'))->toBe('There is a duplication!');
    });

test('validate unique', function () {
        $trans = validatorArrayTranslator();
...
test('validate unique and exists sends correct field name to d b with arrays', function () {
        $trans = validatorArrayTranslator();
...
test('validation exists', function () {
        $trans = validatorArrayTranslator();
...
test('validation exists is not called unnecessarily', function () {
        $trans = validatorArrayTranslator();
...
test('validate gt messages are correct', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines([
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('numeric'))->toEqual('The numeric field must be greater than 10.');
        expect($v->messages()->first('string'))->toEqual('The string field must be greater than 5 characters.');
        expect($v->messages()->first('file'))->toEqual('The file field must be greater than 9 kilobytes.');
        expect($v->messages()->first('array'))->toEqual('The array field must have more than 4 items.');
    });

test('validate gte messages are correct', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines([
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('numeric'))->toEqual('The numeric field must be greater than or equal to 10.');
        expect($v->messages()->first('string'))->toEqual('The string field must be greater than or equal to 5 characters.');
        expect($v->messages()->first('file'))->toEqual('The file field must be greater than or equal to 9 kilobytes.');
        expect($v->messages()->first('array'))->toEqual('The array field must have 4 items or more.');
    });

test('validate lt messages are correct', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines([
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('numeric'))->toEqual('The numeric field must be less than 5.');
        expect($v->messages()->first('string'))->toEqual('The string field must be less than 3 characters.');
        expect($v->messages()->first('file'))->toEqual('The file field must be less than 8 kilobytes.');
        expect($v->messages()->first('array'))->toEqual('The array field must have less than 2 items.');
    });

test('validate lte messages are correct', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines([
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->first('numeric'))->toEqual('The numeric field must be less than or equal to 5.');
        expect($v->messages()->first('string'))->toEqual('The string field must be less than or equal to 3 characters.');
        expect($v->messages()->first('file'))->toEqual('The file field must be less than or equal to 8 kilobytes.');
        expect($v->messages()->first('array'))->toEqual('The array field must not have more than 2 items.');
    });

test('validate ip', function () {
        $trans = validatorArrayTranslator();
...
test('validate mac address', function () {
        $trans = validatorArrayTranslator();
...
test('validate email with international characters', function () {
        $v = new Validator(validatorArrayTranslator(), ['x' => 'foo@gmäil.com'], ['x' => 'email']);
        expect($v->passes())->toBeTrue();
    });

test('validate email with strict check', function () {
        $v = new Validator(validatorArrayTranslator(), ['x' => 'foo@bar '], ['x' => 'email:strict']);
        expect($v->passes())->toBeFalse();
    });

test('validate email with filter check', function () {
        $v = new Validator(validatorArrayTranslator(), ['x' => 'foo@bar'], ['x' => 'email:filter']);
        expect($v->passes())->toBeFalse();

        $v = new Validator(validatorArrayTranslator(), ['x' => 'example@example.com'], ['x' => 'email:filter']);
        expect($v->passes())->toBeTrue();

        // Unicode characters are not allowed
        $v = new Validator(validatorArrayTranslator(), ['x' => 'exämple@example.com'], ['x' => 'email:filter']);
        expect($v->passes())->toBeFalse();

        $v = new Validator(validatorArrayTranslator(), ['x' => 'exämple@exämple.com'], ['x' => 'email:filter']);
        expect($v->passes())->toBeFalse();
    });

test('validate email with filter unicode check', function () {
        $v = new Validator(validatorArrayTranslator(), ['x' => 'foo@bar'], ['x' => 'email:filter_unicode']);
        expect($v->passes())->toBeFalse();

        $v = new Validator(validatorArrayTranslator(), ['x' => 'example@example.com'], ['x' => 'email:filter_unicode']);
        expect($v->passes())->toBeTrue();

        // Any unicode characters are allowed only in local-part
        $v = new Validator(validatorArrayTranslator(), ['x' => 'exämple@example.com'], ['x' => 'email:filter_unicode']);
        expect($v->passes())->toBeTrue();

        $v = new Validator(validatorArrayTranslator(), ['x' => 'exämple@exämple.com'], ['x' => 'email:filter_unicode']);
        expect($v->passes())->toBeFalse();
    });

test('validate email with custom class check', function () {
        $container = Mockery::mock(Vessel::class);
        $container->shouldReceive('make')->with(NoRFCWarningsValidation::class)->andReturn(new NoRFCWarningsValidation);

        $v = new Validator(validatorArrayTranslator(), ['x' => 'foo@bar '], ['x' => 'email:'.NoRFCWarningsValidation::class]);
        $v->setContainer($container);

        expect($v->passes())->toBeFalse();
    });

test('validate url with protocols', function () {
        $trans = validatorArrayTranslator();
...
test('validate url with valid urls', function ($validUrl) {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['x' => $validUrl], ['x' => 'Url']);
        expect($v->passes())->toBeTrue();
    })->with('validUrls');

test('validate url with invalid urls', function ($invalidUrl) {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['x' => $invalidUrl], ['x' => 'Url']);
        expect($v->passes())->toBeFalse();
    })->with('invalidUrls');

test('validate active url', function ($data, $outcome) {
        $trans = validatorArrayTranslator();
        $v = Mockery::mock(
        ...
        expect($v->passes())->toEqual($outcome);
    })->with('activeUrlDataProvider');

test('validate image', function () {
        $trans = validatorArrayTranslator();
...
test('validate image does not allow php extensions on image mime', function () {
        $trans = validatorArrayTranslator();
        $uploadedFile = [__FILE__, '', null, null, true];
...
test('validate image dimensions', function () {
        // Knowing that demo image.png has width = 3 and height = 2
        $uploadedFile = new UploadedFile(__DIR__.'/fixtures/image.png', '', null, null, true);
        $trans = validatorArrayTranslator();
...
test('validate mimetypes', function () {
        $trans = validatorArrayTranslator();
...
test('validate extension', function () {
        $trans = validatorArrayTranslator();
        $uploadedFile = [__FILE__, '', null, null, true];
...
test('validate mime enforces php check', function () {
        $trans = validatorArrayTranslator();
        $uploadedFile = [__FILE__, '', null, null, true];
...
test('empty rules skipped', function () {
        $trans = validatorArrayTranslator();
...
test('validate alpha num', function () {
        $trans = validatorArrayTranslator();
...
test('validate alpha dash', function () {
        $trans = validatorArrayTranslator();
...
test('validate alpha with ascii option', function () {
        $trans = validatorArrayTranslator();
...
test('validate alpha num with ascii option', function () {
        $trans = validatorArrayTranslator();
...
test('validate alpha dash with ascii option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with africa option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with america option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with antarctica option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with arctic option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with asia option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with atlantic option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with australia option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with europe option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with indian option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with pacific option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with u t c option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with all option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with all with b c option', function () {
        $trans = validatorArrayTranslator();
...
test('validate timezone with per country option without specifying country', function () {
        $trans = validatorArrayTranslator();
...
test('validate not regex', function () {
        $trans = validatorArrayTranslator();
...
test('validate date and format', function () {
...
test('date equals respects carbon test now when parameter is relative', function () {
        date_default_timezone_set('UTC');
        $trans = validatorArrayTranslator();
...
test('before and after with format', function () {
        date_default_timezone_set('UTC');
        $trans = validatorArrayTranslator();
...
test('weak before and after', function () {
        date_default_timezone_set('UTC');
        $trans = validatorArrayTranslator();
...
test('sometimes adding rules', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['x' => 'foo'], ['x' => 'Required']);
        $v->sometimes('x', 'Confirmed', function ($i) {
            return $i->x === 'foo';
        });
        expect($v->getRules())->toEqual(['x' => ['Required', 'Confirmed']]);
        ...
        expect($v->getRules())->toEqual(['x' => ['Required'], 'y' => ['Required']]);
        ...
        expect($v->getRules())->toEqual(['x' => ['Required']]);
        ...
        expect($v->getRules())->toEqual(['x' => ['Required', 'Foo', 'Bar']]);
        ...
        expect($v->getRules())->toEqual(['x' => ['Required', 'Foo', 'Bar:Baz']]);
        ...
        expect($v->getRules())->toEqual(['foo.0.name' => ['Required', 'String']]);
    });

test('item aware sometimes adding rules', function () {
        ...
        expect($v->getRules())->toEqual(['users' => ['array'], 'users.0.name' => ['required', 'string'], 'users.1.name' => ['required', 'string']]);
        ...
        expect($v->getRules())->toEqual(['company.users' => ['array'], 'company.users.0.name' => ['required', 'string'], 'company.users.1.name' => ['required', 'string']]);
        ...
        expect($v->getRules())->toEqual(['company' => ['required']]);
        ...
        expect($v->getRules())->toEqual(['users.0' => ['array'], 'users.1' => ['array'], 'users.0.name' => ['required', 'string'], 'users.1.name' => ['required', 'string']]);
        ...
        expect($v->getRules())->toEqual(['company.users.0' => ['array'], 'company.users.1' => ['array'], 'company.users.0.name' => ['required', 'string'], 'company.users.1.name' => ['required', 'string']]);
        ...
        expect($v->getRules())->toEqual(['user.profile.value' => ['required', 'email'], 'user.profile.photo' => ['required', 'mimes:jpg,bmp,png'], 'user.profile.type' => ['required']]);
        ...
        expect($v->getRules())->toEqual(['profiles.0.value' => ['required', 'email'], 'profiles.1.value' => ['required', 'url']]);
        ...
        expect($v->getRules())->toEqual(['profiles.0.value' => ['required', 'email'], 'profiles.1.value' => ['required']]);
        ...
        expect($v->getRules())->toEqual(['profiles.0.value' => ['required'], 'profiles.1.value' => ['required']]);
        ...
        expect($v->getRules())->toEqual(['users.profiles.0.value' => ['required', 'email'], 'users.profiles.1.value' => ['required']]);
        ...
        expect($v->getRules())->toEqual(['user.name' => ['required'], 'user.type' => ['required'], 'user.value' => ['required', 'email']]);
        ...
        expect($v->getRules())->toEqual(['name' => ['required'], 'type' => ['required'], 'value' => ['required', 'email']]);
        ...
        expect($v->getRules())->toEqual(['email' => ['required', 'email']]);
        ...
        expect($v->getRules())->toEqual(['attendee.name' => ['string', 'required'], 'attendee.title' => ['string', 'required'], 'attendee.type' => ['string', 'required']]);
    });

test('validate sometimes implicit each with asterisks before and after', function () {
        $trans = validatorArrayTranslator();
...
test('custom validators', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines(['validation.foo' => 'foo!'], 'en');
        ...
        expect($v->passes())->toBeFalse();
        $v->messages()->setFormat(':message');
        expect($v->messages()->first('name'))->toBe('foo!');
...
test('class based custom validators', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines(['validation.foo' => 'foo!'], 'en');
        $v = new Validator($trans, ['name' => 'taylor'], ['name' => 'foo']);
        $v->setContainer($container = Mockery::mock(Vessel::class));
...
test('class based custom validators using conventional method', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines(['validation.foo' => 'foo!'], 'en');
        $v = new Validator($trans, ['name' => 'taylor'], ['name' => 'foo']);
        $v->setContainer($container = Mockery::mock(Vessel::class));
...
test('custom implicit validators', function () {
        $trans = validatorArrayTranslator();
...
test('custom dependent validators', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans,
            [
                ['name' => 'Jamie', 'age' => 27],
...
test('exception thrown on incorrect parameter count', function () {
        $this->expectException(InvalidArgumentException::class);
...
test('validate implicit each with asterisks', function () {
        $trans = validatorArrayTranslator();
        $data = ['foo' => [5, 10, 15]];
...
test('sometimes on arrays in implicit rules', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->get('names.0.second'))->toEqual(['validation.string']);
    });

test('validate implicit each with asterisks for required non existing key', function () {
        $trans = validatorArrayTranslator();
...
test('parsing array keys with dot', function () {
        $trans = validatorArrayTranslator();
...
test('parsing array keys with dot when testing existence', function () {
        $trans = validatorArrayTranslator();
...
test('passing slash vulnerability', function () {
        $trans = validatorArrayTranslator();
...
test('placeholders are replaced', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->fails())->toBeTrue();
        expect($v->errors()->getMessages())->toHaveKey('foo.bar');
        ...
        expect($v->passes())->toBeTrue();
        expect($v->validated())->toHaveKey('foo.bar');
    });

test('dot placeholders in parameters are replaced in', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->fails())->toBeTrue();
        expect($v->messages()->first())->toBe('The name field is required when user.name is not present.');
        ...
        expect($v->fails())->toBeTrue();
        expect($v->messages()->first())->toBe('The name field is required when user.name / admin.name is not present.');
    });

test('covering empty keys', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => ['' => ['bar' => '']]], ['foo.*.bar' => 'required']);
        expect($v->fails())->toBeTrue();
    });

test('implicit each with asterisks with array values', function () {
        $trans = validatorArrayTranslator();

        $v = new Validator($trans, ['foo' => ['bar.baz' => '']], ['foo' => 'required']);
        expect($v->validated())->toEqual(['foo' => ['bar.baz' => '']]);
    });

test('validate nested array with common parent child key', function () {
        $trans = validatorArrayTranslator();
...
test('validate nested array with non numeric keys', function () {
        $trans = validatorArrayTranslator();
...
test('validate implicit each with asterisks confirmed', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->has('foo.0.password'))->toBeTrue();
        expect($v->messages()->has('foo.1.password'))->toBeTrue();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->has('foo.0.bar.0.password'))->toBeTrue();
        expect($v->messages()->has('foo.0.bar.1.password'))->toBeTrue();
    });

test('validate implicit each with asterisks different', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->has('foo.0.name'))->toBeTrue();
        expect($v->messages()->has('foo.1.name'))->toBeTrue();
        ...
        expect($v->passes())->toBeFalse();
        expect($v->messages()->has('foo.0.bar.0.name'))->toBeTrue();
        expect($v->messages()->has('foo.0.bar.1.name'))->toBeTrue();
    });

test('validate implicit each with asterisks same', function () {
        $trans = validatorArrayTranslator();
...
test('validate implicit each with asterisks required', function () {
        $trans = validatorArrayTranslator();
...
test('validate implicit each with asterisks required if', function () {
        $trans = validatorArrayTranslator();
...
test('validate implicit each with asterisks required unless', function () {
        $trans = validatorArrayTranslator();
...
test('validate implicit each with asterisks required with', function () {
        $trans = validatorArrayTranslator();
...
test('validate implicit each with asterisks required with all', function () {
        $trans = validatorArrayTranslator();
...
test('validate implicit each with asterisks required without', function () {
        $trans = validatorArrayTranslator();
...
test('validate implicit each with asterisks required without all', function () {
        $trans = validatorArrayTranslator();
...
test('validate implicit each with asterisks before and after', function () {
        $trans = validatorArrayTranslator();
...
test('get leading explicit attribute path', function () {
        expect(ValidationData::getLeadingExplicitAttributePath('*.email'))->toBeNull();
        expect(ValidationData::getLeadingExplicitAttributePath('foo.*'))->toBe('foo');
        expect(ValidationData::getLeadingExplicitAttributePath('foo.bar.*.baz'))->toBe('foo.bar');
        expect(ValidationData::getLeadingExplicitAttributePath('foo.bar.1'))->toBe('foo.bar.1');
    });

test('extract data from path', function () {
        $data = [['email' => 'mail'], ['email' => 'mail2']];
        expect(ValidationData::extractDataFromPath(null, $data))->toEqual([['email' => 'mail'], ['email' => 'mail2']]);

        $data = ['cat' => ['cat1' => ['name']], ['cat2' => ['name2']]];
        expect(ValidationData::extractDataFromPath('cat.cat1', $data))->toEqual(['cat' => ['cat1' => ['name']]]);

        $data = ['cat' => ['cat1' => ['name' => '1', 'price' => 1]], ['cat2' => ['name' => 2]]];
        expect(ValidationData::extractDataFromPath('cat.cat1.name', $data))->toEqual(['cat' => ['cat1' => ['name' => '1']]]);
    });

test('using setters with implicit rules', function () {
        $trans = validatorArrayTranslator();
...
test('nested invalid method', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, [
            'testvalid' => 'filled',
            'testinvalid' => '',
...
test('custom validation object', function () {
        // Test passing case...
        $v = new Validator(
            validatorArrayTranslator(),
            ['name' => 'taylor'],
            [
        ...
        expect($v->fails())->toBeTrue();
        expect($v->errors()->all()[0])->toBe('name must be taylor');
        ...
        expect($v->fails())->toBeTrue();
        expect($v->errors()->all()[0])->toBe('name was adam instead of taylor');
        ...
        expect($v->passes())->toBeFalse();
        expect($v->errors()->get('states.0')[0])->toBe('states.0 must be AR or TX');
        expect($v->errors()->get('states.1')[0])->toBe('states.1 must be AR or TX');
        expect($v->errors()->get('number')[0])->toBe('number must be divisible by 4');
        ...
        expect($v->fails())->toBeTrue();
        expect($v->errors()->get('name')[0])->toBe('name must be taylor');
        expect($v->errors()->get('name')[1])->toBe('name must be a first name');
        ...
        expect($v->errors()->get('name')[0])->toBe('name must be taylor');
        expect($v->errors()->get('name')[1])->toBe('name must be a first name');
        expect($v->errors()->get('name')[2])->toBe('validation.string');
        ...
        expect($v->fails())->toBeTrue();
        expect($v->errors()->get('password')[0])->toBe('The password confirmation does not match.');
        ...
        expect($v->fails())->toBeTrue();
        expect($v->errors()->get('double')[0])->toBe('The double must be the double of base.');
        ...
        expect($v->fails())->toBeTrue();
        expect($v->errors()->get('double'))->toHaveCount(1);
        expect($v->errors()->get('double')[0])->toBe('validation.integer');
    });

test('custom validation object with dot keys is correctly passed value', function () {
        $v = new Validator(
            validatorArrayTranslator(),
            ['foo' => ['foo.bar' => 'baz']],
            [
        ...
        expect($v->passes())->toBeFalse();
        expect($v->failed()['foo.foo.bar'])->toBeArray();
    });

test('implicit custom validation objects', function () {
        // Test passing case...
        $v = new Validator(
            validatorArrayTranslator(),
            ['name' => ''],
        ...
        expect($v->passes())->toBeTrue();
        expect($rule->called)->toBeTrue();
    });

test('validate returns validated data', function () {
        $post = ['first' => 'john', 'preferred' => 'john', 'last' => 'doe', 'type' => 'admin'];

        $v = new Validator(validatorArrayTranslator(), $post, ['first' => 'required', 'preferred' => 'required']);
        $v->sometimes('type', 'required', function () {
            return false;
        });
        $data = $v->validate();

        expect($data)->toEqual(['first' => 'john', 'preferred' => 'john']);
    });

test('validate returns validated data nested rules', function () {
        $post = ['nested' => ['foo' => 'bar', 'baz' => ''], 'array' => [1, 2]];

        $rules = ['nested.foo' => 'required', 'array.*' => 'integer'];

        $v = new Validator(validatorArrayTranslator(), $post, $rules);
        $v->sometimes('type', 'required', function () {
            return false;
        });
        $data = $v->validate();

        expect($data)->toEqual(['nested' => ['foo' => 'bar'], 'array' => [1, 2]]);
    });

test('validate returns validated data nested child rules', function () {
        $post = ['nested' => ['foo' => 'bar', 'with' => 'extras', 'type' => 'admin']];

        $v = new Validator(validatorArrayTranslator(), $post, ['nested.foo' => 'required']);
        $v->sometimes('nested.type', 'required', function () {
            return false;
        });
        $data = $v->validate();

        expect($data)->toEqual(['nested' => ['foo' => 'bar']]);
    });

test('validate returns validated data nested array rules', function () {
        $post = ['nested' => [['bar' => 'baz', 'with' => 'extras', 'type' => 'admin'], ['bar' => 'baz2', 'with' => 'extras', 'type' => 'admin']]];

        $v = new Validator(validatorArrayTranslator(), $post, ['nested.*.bar' => 'required']);
        $v->sometimes('nested.*.type', 'required', function () {
            return false;
        });
        $data = $v->validate();

        expect($data)->toEqual(['nested' => [['bar' => 'baz'], ['bar' => 'baz2']]]);
    });

test('validate and validated data', function () {
        $post = ['first' => 'john', 'preferred' => 'john', 'last' => 'doe', 'type' => 'admin'];
...
test('validated not validate twice data', function () {
        $post = ['first' => 'john', 'preferred' => 'john', 'last' => 'doe', 'type' => 'admin'];
        ...
        expect($data)->toEqual(['first' => 'john', 'preferred' => 'john']);
        expect($validateCount)->toEqual(1);
    });

test('multiple passes calls', function () {
        $trans = validatorArrayTranslator();
...
test('validate with valid uuid', function ($uuid) {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => $uuid], ['foo' => 'uuid']);
        expect($v->passes())->toBeTrue();
    })->with('validUuidList');

test('validate with invalid uuid', function ($uuid) {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => $uuid], ['foo' => 'uuid']);
        expect($v->passes())->toBeFalse();
    })->with('invalidUuidList');

test('validate with uuid with version constraint', function ($uuid, $rule, $passes) {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => $uuid], ['foo' => $rule]);
        expect($passes)->toBe($v->passes());
    })->with('uuidVersionList');

test('validate with valid ascii', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => 'Dusseldorf'], ['foo' => 'ascii']);
        expect($v->passes())->toBeTrue();
    });

test('validate with invalid ascii', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => 'Düsseldorf'], ['foo' => 'ascii']);
        expect($v->passes())->toBeFalse();
    });

test('validate with valid ulid', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => '01gd6r360bp37zj17nxb55yv40'], ['foo' => 'ulid']);
        expect($v->passes())->toBeTrue();
    });

test('validate with invalid ulid', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['foo' => '01gd6r36-bp37z-17nx-55yv40'], ['foo' => 'ulid']);
        expect($v->passes())->toBeFalse();
    });

test('exclude if', function ($rules, $data, $expectedValidatedData) {
        ...
        expect($validator->validated())->toBe($expectedValidatedData);
    })->with('providesPassingExcludeIfData');

test('exclude if when validation fails', function ($rules, $data, $expectedMessages) {
        $validator = new Validator(
            validatorArrayTranslator(),
            $data,
            $rules
        ...
        expect($validator->messages()->toArray())->toBe($expectedMessages);
    })->with('providesFailingExcludeIfData');

test('exclude', function ($rules, $data, $expectedValidatedData) {
        $validator = new Validator(
            validatorArrayTranslator(),
            $data,
            $rules
        ...
        expect($validator->validated())->toBe($expectedValidatedData);
    })->with('providesPassingExcludeData');

test('exclude before a dependent rule', function () {
        $validator = new Validator(
            validatorArrayTranslator(),
            [
                'profile_id' => null,
        ...
        expect($validator->validated())->toBe(['profile_id' => null]);
        ...
        expect($validator->passes())->toBeFalse();
        expect($validator->getMessageBag()->getMessages())->toBe(['profile_id' => ['validation.required_if']]);
    });

test('excluding arrays', function () {
        $validator = new Validator(
            validatorArrayTranslator(),
        ...
        expect($validator->passes())->toBeTrue();
        expect($validator->validated())->toBe(['users' => [['name' => 'Mohamed', 'location' => 'cairo']]]);
        ...
        expect($validator->passes())->toBeTrue();
        expect($validator->validated())->toBe(['users' => [['name' => 'Mohamed']]]);
        ...
        $validator->excludeUnvalidatedArrayKeys = true;
        expect($validator->passes())->toBeTrue();
        expect($validator->validated())->toBe(['admin' => ['name' => 'Mohamed'], 'users' => [['name' => 'Mohamed']]]);
        ...
        $validator->excludeUnvalidatedArrayKeys = true;
        expect($validator->passes())->toBeTrue();
        expect($validator->validated())->toBe(['users' => ['mohamed', 'zain']]);
        ...
        $validator->excludeUnvalidatedArrayKeys = true;
        expect($validator->passes())->toBeTrue();
        expect($validator->validated())->toBe(['users' => ['admins' => [['name' => 'mohamed']]]]);
        ...
        $validator->excludeUnvalidatedArrayKeys = true;
        expect($validator->passes())->toBeTrue();
        expect($validator->validated())->toBe(['users' => [1, 2, 3]]);
...
test('exclude unless', function () {
        ...
        expect($validator->passes())->toBeTrue();
        expect($validator->validated())->toBe(['cat' => 'Felix']);
        ...
        expect($validator->passes())->toBeTrue();
        expect($validator->validated())->toBe(['cat' => 'Tom', 'mouse' => 'Jerry']);
        ...
        expect($validator->messages()->toArray())->toBe(['mouse' => ['validation.required']]);
        ...
        expect($validator->passes())->toBeTrue();
        expect($validator->validated())->toBe(['foo' => true]);
        ...
        expect($v->passes())->toBeTrue();
        expect($v->validated())->toBe(['bar' => 'Hello']);
    });

test('exclude without', function () {
        $validator = new Validator(
            validatorArrayTranslator(),
            ['region' => 'South'],
            [
        ...
        expect($validator->fails())->toBeTrue();
        expect($validator->messages()->toArray())->toBe(['country' => ['validation.required_with']]);
    });

test('exclude values are really removed', function () {
        $validator = new Validator(
            validatorArrayTranslator(),
            ['cat' => 'Tom', 'mouse' => 'Jerry'],
        ...
        expect($validator->passes())->toBeTrue();
        expect($validator->validated())->toBe(['cat' => 'Tom']);
        expect($validator->valid())->toBe(['cat' => 'Tom']);
        ...
        expect($validator->fails())->toBeTrue();
        expect($validator->valid())->toBe(['cat' => 'Tom']);
        expect($validator->invalid())->toBe(['mouse' => null]);
    });

test('exclude with values are really removed', function () {
        $validator = new Validator(
            validatorArrayTranslator(),
            [
                'cat' => 'Tom',
...
test('validate fails with asterisks as data keys', function () {
        $post = ['data' => [0 => ['date' => '2019-01-24'], 1 => ['date' => 'blah'], '*' => ['date' => 'blah']]];

        $rules = ['data.*.date' => 'required|date'];

        $validator = new Validator(validatorArrayTranslator(), $post, $rules);

        expect($validator->fails())->toBeTrue();
        expect($validator->messages()->toArray())->toBe(['data.1.date' => ['validation.date'], 'data.*.date' => ['validation.date']]);
    });

test('fail on first error', function () {
        $trans = validatorArrayTranslator();
        $data = [
            'foo' => 'bar',
            'age' => 30,
        ...
        expect($failOnFirstErrorDisable->passes())->toBeFalse();
        expect($failOnFirstErrorDisable->getMessageBag()->getMessages())->toEqual($expectedFailOnFirstErrorDisableResult);
        ...
        expect($failOnFirstErrorEnable->passes())->toBeFalse();
        expect($failOnFirstErrorEnable->getMessageBag()->getMessages())->toEqual($expectedFailOnFirstErrorEnableResult);
    });

test('array keys validation passed when has keys', function () {
        $trans = validatorArrayTranslator();
...
test('array keys validation passed with partial match', function () {
        $trans = validatorArrayTranslator();
...
test('array keys validation fails with missing key', function () {
        $trans = validatorArrayTranslator();
        ...
        $validator = new Validator($trans, $data, $rules, [], []);
        expect($validator->passes())->toBeFalse();
        expect($validator->messages()->first('baz'))->toBe('The baz field must contain entries for foo, fee, boo, bar');
    });

test('array keys validation fails with not an array', function () {
        $trans = validatorArrayTranslator();
        $trans->addLines(['validation.required_array_keys' => 'The :attribute field must contain entries for :values'], 'en');
...
test('array keys with dot integer min', function () {
        $trans = validatorArrayTranslator();
        ...
        $validator = new Validator($trans, $data, $rules, [], []);
        expect($validator->getMessageBag()->getMessages())->toEqual($expectedResult);
    });

test('it can translate messages for closure based rules', function () {
        $trans = validatorArrayTranslator();
...
test('it can specify the validation error key for the error message for closure based rules', function () {
        $trans = validatorArrayTranslator();
        $rule = function ($attribute, $value, $fail) {
...
test('it trims space from parameters', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($validator->messages()->keys())->toBe([
            'min',
            'min_str',
            'multiple_of',
            'between',
...
test('it limits length of scientific notation exponent', function ($value) {
        $trans = validatorArrayTranslator();
        $validator = new Validator($trans, ['foo' => $value], ['foo' => 'numeric|min:3']);

        expect($validator->passes())->toBeFalse();
    })->with('outsideRangeExponents');

test('it allows scientific notation within range', function ($value, $rule) {
        $trans = validatorArrayTranslator();
        $validator = new Validator($trans, ['foo' => $value], ['foo' => ['numeric', $rule]]);

        expect($validator->passes())->toBeTrue();
    })->with('withinRangeExponents');

test('it can configure allowed exponent range', function () {
        $trans = validatorArrayTranslator();
        ...
        expect($attribute)->toBe('foo');
        expect($value)->toBe('1.0e-1000');
...
test('messages default when using size specific custom messages', function () {
        $trans = validatorArrayTranslator();
...
test('when fails', function () {
        ...
        expect($result)->toBe('whenNotFails');
    });

test('when passes', function () {
        $trans = validatorArrayTranslator();
        $v = new Validator($trans, ['text' => 'abc'], ['text' => 'string|max:5']);
        ...
        expect($result)->toBe('whenNotPasses');
    });

dataset('prohibitedRulesData', function () {
...
dataset('multipleOfDataProvider', function () {
        return [
            [0, 0, false], // zero (same)
...
dataset('activeUrlDataProvider', function () {
        return [
            'Invalid Url' => [
                ['x' => 'aslsdlks'],
                false,
...
dataset('validUuidList', function () {
        return [
...
dataset('invalidUuidList', function () {
        return [
...
dataset('uuidVersionList', function () {
        return [
...
dataset('providesPassingExcludeIfData', function () {
        return [
            [
                [
...
dataset('providesFailingExcludeIfData', function () {
        return [
            [
                [
                    'has_appointment' => ['required', 'bool'],
...
dataset('providesPassingExcludeData', function () {
        return [
            [
                [
                    'has_appointment' => ['required', 'bool'],
...
dataset('outsideRangeExponents', function () {
        return [
            ['1.0e+1001'],
            ['1.0E+1001'],
            ['1.0e1001'],
...
dataset('withinRangeExponents', function () {
        return [
            ['1.0e+1000', 'min:3'],
            ['1.0E+1000', 'min:3'],
            ['1.0e1000', 'min:3'],
