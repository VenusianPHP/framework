...
class ValidationValidatorTest extends TestCase
{
    ...
    public function testClassBasedCustomReplacers()
    {
        ...
        $v->setContainer($container = m::mock(Vessel::class));
    ...
    // Laravel covers the `current_password` rule here with four tests built on
    // Auth's Guard and Authenticatable. Auth is out of scope for this port, so
    // the rule's tests are cut with it.
    ...
    {
        $v = new Validator($this->getVoyagerArrayTranslator(), ['x' => 'foo@gmäil.com'], ['x' => 'email']);
        $this->assertTrue($v->passes());
    }

    public function testValidateEmailWithStrictCheck()
    {
        $v = new Validator($this->getVoyagerArrayTranslator(), ['x' => 'foo@bar '], ['x' => 'email:strict']);
        $this->assertFalse($v->passes());
    }

    public function testValidateEmailWithFilterCheck()
    {
        $v = new Validator($this->getVoyagerArrayTranslator(), ['x' => 'foo@bar'], ['x' => 'email:filter']);
        $this->assertFalse($v->passes());

        $v = new Validator($this->getVoyagerArrayTranslator(), ['x' => 'example@example.com'], ['x' => 'email:filter']);
        $this->assertTrue($v->passes());

        // Unicode characters are not allowed
        $v = new Validator($this->getVoyagerArrayTranslator(), ['x' => 'exämple@example.com'], ['x' => 'email:filter']);
        $this->assertFalse($v->passes());

        $v = new Validator($this->getVoyagerArrayTranslator(), ['x' => 'exämple@exämple.com'], ['x' => 'email:filter']);
        $this->assertFalse($v->passes());
    }

    public function testValidateEmailWithFilterUnicodeCheck()
    {
        $v = new Validator($this->getVoyagerArrayTranslator(), ['x' => 'foo@bar'], ['x' => 'email:filter_unicode']);
        $this->assertFalse($v->passes());

        $v = new Validator($this->getVoyagerArrayTranslator(), ['x' => 'example@example.com'], ['x' => 'email:filter_unicode']);
        $this->assertTrue($v->passes());

        // Any unicode characters are allowed only in local-part
        $v = new Validator($this->getVoyagerArrayTranslator(), ['x' => 'exämple@example.com'], ['x' => 'email:filter_unicode']);
        $this->assertTrue($v->passes());

        $v = new Validator($this->getVoyagerArrayTranslator(), ['x' => 'exämple@exämple.com'], ['x' => 'email:filter_unicode']);
        $this->assertFalse($v->passes());
    }
    ...
    public function testValidateEmailWithCustomClassCheck()
    {
        ...
        $v = new Validator($this->getVoyagerArrayTranslator(), ['x' => 'foo@bar '], ['x' => 'email:'.NoRFCWarningsValidation::class]);
...
1',
    ...
    // Laravel covers `parseTable()` against Instrument models here. Instrument
    // arrives in wave 6; this test and its model fixtures come back with it.
    ...
    public function testValidateReturnsValidatedData()
    {
        ...
        $v = new Validator($this->getVoyagerArrayTranslator(), $post, ['first' => 'required', 'preferred' => 'required']);
    ...
    public function testValidateReturnsValidatedDataNestedRules()
    {
        ...
        $v = new Validator($this->getVoyagerArrayTranslator(), $post, $rules);
        $v->sometimes('type', 'required', function () {
            return false;
        });
        $data = $v->validate();
    ...
    public function testValidateReturnsValidatedDataNestedChildRules()
    {
        ...
        $v = new Validator($this->getVoyagerArrayTranslator(), $post, ['nested.foo' => 'required']);
    ...
    public function testValidateReturnsValidatedDataNestedArrayRules()
    {
        ...
        $v = new Validator($this->getVoyagerArrayTranslator(), $post, ['nested.*.bar' => 'required']);
        $v->sometimes('nested.*.type', 'required', function () {
            return false;
        });
        $data = $v->validate();
    ...
    public function testValidateFailsWithAsterisksAsDataKeys()
    {
        ...
        $validator = new Validator($this->getVoyagerArrayTranslator(), $post, $rules);
